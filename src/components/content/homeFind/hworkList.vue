...
import { getUserInfo, getMyStudentHworkList } from '../../../service/user/user'
...
export default {
  ...
  methods: {
    ...
    async getMyStudentHworkList() {
      try {
        const data = await getMyStudentHworkList()
        if (data.status===200) {
