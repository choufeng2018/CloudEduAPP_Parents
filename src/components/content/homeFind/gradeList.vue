  ...
  <div class="nearby-stores">
    <div :key="index" class="nearby-stores-card flex" v-for="(item, index) in studetScoreList">
      ...
      <div class="nearby-stores-card--content">
        ...
        <q-btn label="编辑" @click="getDetail(item.scoreId)" />
...
import { getUserInfo, getMyStudentScoreList } from '../../../service/user/user'
...
export default {
  ...
  methods: {
    ...
    async getMyStudentScoreList() {
      try {
        const data = await getMyStudentScoreList()
        if (data.status===200) {
    ...
    getDetail(id) {
      this.$router.push({path:'/addGrade',query:{id:id}})
    },
    cardActions(type) {
			this.$router.push({
				path: '/addGrade'
