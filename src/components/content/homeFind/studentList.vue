<template>
  <div class="nearby-stores">
    <div :key="index" class="nearby-stores-card flex" v-for="(item, index) in studetList">
...
import { getUserInfo, getMyStudentList } from '../../../service/user/user'
...
export default {
  ...
  methods: {
    ...
    async getMyStudentList() {
      try {
        const data = await getMyStudentList()
        if (data.status===200) {
         this.studetList = data.data.rows
         console.log(this.studetList)
        }
      } catch (error) {}
    },
    getDetail(id) {
      this.$router.push({path:'/addHeight',query:{id:id}})
    },
    cardActions(type) {
			this.$router.push({
				path: '/addStudent'
