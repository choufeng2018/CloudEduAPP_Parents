<template>
  <div class="nearby-stores">
    <div :key="index" class="nearby-stores-card flex" v-for="(item, index) in studetScoreList">
...
import { GETUSEINFO_ACTION } from '../../../store/homeWork/action-types.js'
import { getUserInfo, getMyStudentMisList } from '../../../service/user/user'
import { GETUSERINFO_MUTATION } from '../../../store/homeWork/mutation-types.js'
export default {
  name: 'StudentList',
  data() {
    return {
  ...
  methods: {
    ...
    async getMyStudentMisList() {
      try {
        const data = await getMyStudentMisList()
        if (data.status===200) {
         this.studetScoreList = data.data.rows
         console.log(this.studetHwList)
        }
      } catch (error) {}
    },
    getDetail(id) {
      this.$router.push({path:'/errorBook',query:{id:id}})
    },
    cardActions(type) {
			this.$router.push({
				path: '/errorBook'
