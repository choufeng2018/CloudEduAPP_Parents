...
import { getUserInfo, getMyStudentLeaveList } from '../../../service/user/user'
...
export default {
  ...
  methods: {
    ...
    async getMyStudentLeaveList() {
      try {
        const data = await getMyStudentLeaveList()
        if (data.status===200) {
    ...
    getDetail(id) {
      this.$router.push({path:'/askLeave',query:{id:id}})
    },
    cardActions(type) {
			this.$router.push({
				path: '/askLeave'
